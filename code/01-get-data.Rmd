...
👀Checkout the [Bioconductor SRAdb package](https://bioconductor.org/packages/release/bioc/html/SRAdb.html)
...
## Do we use SRAdb R package? 

```{r}
browseVignettes("SRAdb")
```

We have to get the SRAdb SQLite file from the online location. The download
and uncompress steps are done automatically with a single command, `getSRAdbFile( )`, which has defaults `getSRAdbFile( destdir = getwd(), destfile = "SRAmetadb.sqlite.gz", method )`
