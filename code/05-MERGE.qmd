...
```{r}
# Select top 20 differentially expressed genes from the ordered matrix
top_genes <- row.names(acs_ordered_matrix)[1:20]
kable(top_genes)

# Identify top genes that are present in the counts_matrix_unico row names
present_genes <- intersect(top_genes, row.names(acs_ordered_matrix))
